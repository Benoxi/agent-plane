...
- `vp run dist:desktop:fedora` — Builds a Fedora AppImage into `./release-fedora`.
- `vp run install:desktop:fedora` — Installs the latest Fedora AppImage into `~/.local/share/t3code/stable` and creates a launcher that shares the default `~/.t3` state with the installed desktop app.
- `vp run refresh:desktop:fedora` — Builds, installs, and launches a refreshed Fedora AppImage.
