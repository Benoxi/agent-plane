...
describe("isComposerPayloadSnapshotCurrent", () => {
  it("only matches an unchanged prompt and attachment identity snapshot", () => {
    const snapshot = { snapshotPrompt: "Review this", snapshotItemIds: [["image-1"], []] };
    expect(
      isComposerPayloadSnapshotCurrent({
        ...snapshot,
        currentPrompt: "Review this",
        currentItemIds: [["image-1"], []],
      }),
    ).toBe(true);
    expect(
      isComposerPayloadSnapshotCurrent({
        ...snapshot,
        currentPrompt: "Review this and more",
        currentItemIds: [["image-1"], []],
      }),
    ).toBe(false);
  });
