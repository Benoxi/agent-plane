...
describe("resolveThreadRowShortcut", () => {
  ...
  it("maps focused row activation, rename, and delete keys", () => {
    expect(resolve("Enter")).toBe("activate");
    expect(resolve(" ")).toBe("activate");
    expect(resolve("F2")).toBe("rename");
    expect(resolve("Delete")).toBe("delete");
  });

  it("ignores nested targets, rename mode, and repeated destructive shortcuts", () => {
    expect(resolve("Delete", { isRowTarget: false })).toBeNull();
    expect(resolve("F2", { isRenaming: true })).toBeNull();
    expect(resolve("Delete", { repeat: true })).toBeNull();
  });
});
import {
  EnvironmentId,
