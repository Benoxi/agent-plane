...
export default function Sidebar() {
  ...
  const [claudeImportPickerGroup, setClaudeImportPickerGroup] =
    useState<SidebarProjectSnapshot | null>(null);
  const [claudeImportTarget, setClaudeImportTarget] = useState<ClaudeSessionImportTarget | null>(
    null,
  );
  const handleClaudeImport = useCallback(
    (event: ReactMouseEvent<HTMLElement>, project: SidebarProjectSnapshot) => {
      event.preventDefault();
      event.stopPropagation();
      dispatchProjectScopeMenu({ type: "open-changed", open: false });
      window.requestAnimationFrame(() => {
        if (project.memberProjects.length === 1) {
          setClaudeImportTarget(claudeSessionImportTargetForProject(project.memberProjects[0]!));
          return;
        }
        setClaudeImportPickerGroup(project);
      });
    },
    [],
  );
  ...
  return (
    <>
                            ...
                            {project ? (
                              <span className="ml-auto flex items-center gap-0.5">
                                <Button
                                  ...
                                  variant="ghost-muted"
                                  tabIndex={-1}
                                  title={`Import Claude thread into ${project.displayName}`}
                                  aria-label={`Import Claude thread into ${project.displayName}`}
                                  className="size-6 [--control-icon-color:currentColor] text-icon-muted focus-visible:bg-accent focus-visible:text-foreground"
                                  onPointerDown={(event) => event.stopPropagation()}
                                  onClick={(event) => handleClaudeImport(event, project)}
                                >
      ...
      <Dialog
        open={claudeImportPickerGroup !== null}
        onOpenChange={(open) => {
          if (!open) setClaudeImportPickerGroup(null);
        }}
      >
        <DialogPopup>
          <DialogHeader>
            <DialogTitle>Choose a checkout</DialogTitle>
            <DialogDescription>
              Import a Claude thread into one checkout in {claudeImportPickerGroup?.displayName}.
            </DialogDescription>
          </DialogHeader>
          <DialogPanel className="grid gap-2">
            {claudeImportPickerGroup?.memberProjects.map((member) => (
              <Button
                key={member.physicalProjectKey}
                variant="outline"
                className="h-auto min-w-0 justify-start px-3 py-2 text-left"
                onClick={() => {
                  setClaudeImportPickerGroup(null);
                  setClaudeImportTarget(claudeSessionImportTargetForProject(member));
                }}
              >
                <span className="min-w-0">
                  <span className="block truncate">{member.title}</span>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setClaudeImportPickerGroup(null)}>
              Cancel
            </Button>
          </DialogFooter>
        </DialogPopup>
