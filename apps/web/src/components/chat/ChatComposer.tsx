...
import {
  ...
  ProviderUsageScheduleContext,
} from "../usage/ProviderUsageIndicator";
...
const ComposerFooterPrimaryActions = memo(function ComposerFooterPrimaryActions(props: {
  ...
  return (
    <>
      ...
      {!props.pendingAction && !props.isRunning && !props.showPlanFollowUpPrompt ? (
        <ComposerVoiceDictationButton {...props.voiceDictation} />
      ) : null}
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const scheduleDisabledReason =
    routeKind !== "server" || !activeThreadId
      ? "Start the thread before scheduling messages."
      : !composerSendState.hasSendableContent
        ? "Add text, images, or context to schedule it."
        : null;
  ...
  const cancelVoiceDictation = useCallback(() => {
    ...
    recognition?.dispose({ abort: true });
  ...
  const toggleVoiceDictation = useCallback(() => {
    ...
    try {
      recognition = createComposerSpeechRecognition({
        ...
        onError: (message) => {
          if (speechRecognitionRef.current === recognition) cancelVoiceDictation();
          toastManager.add({
            type: "error",
        ...
        onEnd: () => {
          ...
          setVoiceListening(false);
          setVoiceElapsedSeconds(0);
          if (voiceFinalTextRef.current) {
            insertComposerTextAtEnd(voiceFinalTextRef.current, { ensureLeadingBoundary: true });
          }
        },
      });
      ...
      toastManager.add({
        ...
        description: cause instanceof Error ? cause.message : "Voice dictation is unavailable.",
      });
    }
  }, [cancelVoiceDictation, insertComposerTextAtEnd]);
  useEffect(() => {
  ...
  useEffect(() => cancelVoiceDictation, [cancelVoiceDictation]);
  const voiceSupport = detectSpeechRecognitionSupport();
  const voiceDictation = {
    disabled: isConnecting || isSendBusy || isPreparingWorktree,
    unsupportedReason: voiceSupport.supported
      ? null
      : voiceSupport.reason === "insecure-context"
